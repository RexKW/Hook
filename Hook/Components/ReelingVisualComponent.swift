...
class ReelingVisualComponent: GKComponent {
    ...
    func triggerFailureEffect() {
        let normalWheelColor = UIColor.darkGray
        let normalTargetColor = UIColor.green
        let darkRed = UIColor(red: 0.6, green: 0.0, blue: 0.0, alpha: 1.0)
        
        // 1. Force reset positions and colors to baseline immediately
        wheelNode.strokeColor = normalWheelColor
        targetZoneNode?.strokeColor = normalTargetColor
        wheelNode.position = .zero
        targetZoneNode?.position = .zero
        
        // 2. Define the color change logic
        let turnRed = SKAction.run {
            self.wheelNode.strokeColor = darkRed
            self.targetZoneNode?.strokeColor = .red
        }
        let waitDelay = SKAction.wait(forDuration: 0.3)
        let revertColor = SKAction.run {
            self.wheelNode.strokeColor = normalWheelColor
            self.targetZoneNode?.strokeColor = normalTargetColor
        }
        let colorSequence = SKAction.sequence([turnRed, waitDelay, revertColor])
        
        // 3. Define the shake logic
        ...
        // Reset position at the end of the shake
        let resetPosition = SKAction.run {
            self.wheelNode.position = .zero
            self.targetZoneNode?.position = .zero
        }
        
        let singleShake = SKAction.sequence([moveLeft, moveRight, moveCenter])
        let shakeSequence = SKAction.sequence([SKAction.repeat(singleShake, count: 2), resetPosition])
        
        // 4. Run the combined effect
